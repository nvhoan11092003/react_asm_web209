...
import {signupSchema, signinSchema, forgotPasswordSchema, changePasswordSchema} from "../schema/user"
...
export const signin = async (req,res) => {
    try {
        ...
        const token = jwt.sign({_id : checkUser._id}, "team 5", {expiresIn: "1h"})
...
export const changePassword = async (req, res) => {
    try {
        ...
        const {error} = changePasswordSchema.validate({email,password,newPassword},{abortEarly:false})
    if(error){
        const errors = error.details.map((err)=>err.message)
        return res.status(400).json({
            message: errors

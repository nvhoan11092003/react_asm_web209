...
import { getAllImages, updateImage } from "../controller/upload.js";
...
router.post(
  "/upload",
  upload.fields([{ name: "img", maxCount: 5 }]),
  (req, res) => {
    if (req.files["img"]) {
      ...
      for (let i = 0; i < images.length; i++) {
        if (images[i].size <= 2 * 1024 * 1024) {
          ...
          return res
            .status(400)
            .json({ error: "Kích thước tệp ảnh không được vượt quá 2MB" });
        }
      }
